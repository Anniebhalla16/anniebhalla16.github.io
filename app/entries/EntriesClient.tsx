...
export default function EntriesClient({ entries, categories }: Props) {
  ...
  const filtered = useMemo(() =>
    activeCat === 'all' ? entries : entries.filter(e => e.categories.key === activeCat),
  [activeCat, entries])
  ...
  const counts = useMemo(() => {
    ...
    entries.forEach(e => { c[e.categories.key] = (c[e.categories.key] ?? 0) + 1 })
    return c
  }, [entries])
        ...
        <main className="entries-main">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 'clamp(24px,4vh,40px)', paddingBottom: 16, borderBottom: `1px solid ${P.hairline}` }}>
            <span style={{ fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: P.muted }}>
              {activeCat === 'all' ? 'All entries' : catsMap[activeCat]?.label ?? activeCat}
            </span>
          ...
          {activeCat === 'all' && featured && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <CategoryChip label={featured.categories.label} color={featured.categories.color} size="sm" />
                <span style={{ fontSize: 10, letterSpacing: '.14em', textTransform: 'uppercase', color: P.cognac, opacity: 0.7 }}>
                  Featured
                </span>
                <span style={{ fontSize: 11, color: P.muted, marginLeft: 'auto' }}>{featured.date} · {featured.read_time} min</span>
              </div>
